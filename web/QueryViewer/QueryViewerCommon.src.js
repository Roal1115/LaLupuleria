...
qv.card = (function () {
	...
	function renderCard(qViewer) {
		...
		function TrendDiv(parent, qViewer, data) {
			var trendClass;
			...
			var styleObj = { width: "16px", height: "16px" };
			if (qViewer.IncludeSparkline)
				styleObj.marginLeft = "5px";
			else
				styleObj.marginRight = "5px";
			var div = qv.util.dom.createDiv(parent, "", trendClass, trendTooltip, styleObj, "");
			return div;
		}
		...
		var container = qv.util.dom.getEmptyContainer(qViewer);
		var tableOuter = qv.util.dom.createTable(container, qv.util.GetContainerControlClasses(qViewer), { width: gx.dom.addUnits(qViewer.Width), height: gx.dom.addUnits(qViewer.Height) });		// Contenedor del UC
		var trOuter = qv.util.dom.createRow(tableOuter);
		var tdOuter = qv.util.dom.createCell(trOuter, 1, "center", {}, "");
		var width = qViewer.Orientation == "Horizontal" ? "100%" : "";
		var height = qViewer.Orientation == "Vertical" ? "100%" : "";
		...
		for (var i = 0; i < qViewer.Metadata.Data.length; i++) {
			...
			if (qViewer.ItemClick) {
				...
				onClick = function () { qv.card.fireItemClickEvent(qViewer, datum.Name, aggregateRows) };
			}
			if (qViewer.Orientation == "Vertical")
				trInner = qv.util.dom.createRow(tableInner);
			var paddingRight = "";
			var paddingBottom = "";
			if (i < qViewer.Metadata.Data.length - 1) {
				paddingRight = qViewer.Orientation == "Horizontal" ? "20px" : "";
				paddingBottom = qViewer.Orientation == "Vertical" ? "20px" : "";
			}
			var tdInner = qv.util.dom.createCell(trInner, 1, "center", { paddingRight: paddingRight, paddingBottom: paddingBottom }, "");
			...
			if (anyRows && qViewer.IncludeTrend && !qViewer.IncludeSparkline && data.LinearRegression.AnyTrend)
				TrendDiv(td2, qViewer, data);
			td2 = qv.util.dom.createCell(tr2, 1, "center", {}, "");
			var span2 = qv.util.dom.createSpan(td2, "", elementValueClass, (xDataField != "" ? ageStr : ""), styleObj, onClick, valueStr);
